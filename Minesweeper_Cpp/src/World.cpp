...
World::World(bool _debug, string aiType, string filename) {
  ...
  // True for file provided; false for file not provided, board with default
  // size and random feature
  if (!filename.empty()) {
  ...
  if (aiType == "randomAI")
    agent =
        new RandomAI(rowDimension, colDimension, totalMines, agentX, agentY);

  else if (aiType == "manualAI")
    agent =
        new ManualAI(rowDimension, colDimension, totalMines, agentX, agentY);
...
void World::addMine()
// Generate mine: totalMines times, in bound, no mine before -> [mc][mr]mine =
// true, not adding mine around and on the first move uncover tile
{
  for (int m = 0; m < totalMines; ++m) {
    ...
    while (!isInBounds(mc, mr) || board[mc][mr].mine ||
           ((agentX - 2 < mc && mc < agentX + 2) &&
            (agentY - 2 < mr && mr < agentY + 2))) {
...
void World::addNeighbour(int c, int r) {
  ...
  // iterate 8 neighbours around a tile, and increment neighbour if there is
  // mine
...
int World::randomInt(int limit) { return rand() % limit; }
