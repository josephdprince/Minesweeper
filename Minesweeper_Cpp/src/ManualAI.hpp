...
public:
  ManualAI(int _rowDimension, int _colDimension, int _totalMines, int _agentX,
           int _agentY) {
  ...
  Action getAction(int number) override {
    ...
    std::cout << "---------------- Available Actions ----------------"
              << std::endl;
    std::cout << "L: leave game   U: uncover tile   F: flag   N: unflag"
              << std::endl;
    ...
    if (new_action != LEAVE) {
      std::cout << "Enter X: ";
      while (!(std::cin >> input_x) || !std::cin.good() || input_x < 1 ||
             input_x > rowDimension) {
