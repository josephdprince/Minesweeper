...
//                         1) Treats the InputFile as a folder containing many
//                         worlds. The program will then construct a world for
//                         every valid world file found. The program to display
//                         total score instead of a single score. The InputFile
//                         operand must be specified with this option 2) Threats
//                         the inputFile as a file. The program will then
//                         construct a world for a single valid world file
//                         found. The program to display a single score.
//
...
int main(int argc, char *argv[]) {
  ...
  if (firstToken[0] == '-') {
    ...
    for (int index = 1; index < firstToken.size(); ++index) {
      ...
      if (firstToken[index] == 'r' || firstToken[index] == 'R') {
        if (aiType == "manualAI")
          cout << "[WARNING] Manual AI and Random AI both on;"
                  " Manual AI was turned off."
               << endl;
        aiType = "randomAI";
      }
      if (firstToken[index] == 'm' || firstToken[index] == 'M') {
        if (aiType == "randomAI")
          cout << "[WARNING] Manual AI and Random AI both on; Manual AI was "
                  "turned off."
               << endl;
        else
          aiType = "manualAI";
  ...
  if (worldFile == "") {
    if (folder)
      cout << "[WARNING] No folder specified; running on a random world."
           << endl;
